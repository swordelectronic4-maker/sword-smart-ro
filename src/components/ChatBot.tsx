...
export default function ChatBot() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'welcome',
      role: 'assistant',
      content: 'Hello! Welcome to SWORD Smart Water. I am your AI assistant. Ask me anything about our purifier — pricing, technology, features, or maintenance!',
      timestamp: new Date(),
    },
  ]);
  ...
  const handleLeadSubmit = (name: string, email: string, phone: string) => {
    ...
    if (success) {
      addMessage(
        'assistant',
        `Thank you ${name}! We have received your details. Our SWORD specialist will contact you within 24 hours at ${phone} or ${email}. Have a great day!`
      );
    } else {
      addMessage(
        'assistant',
...
// ═══════════════════════════════════════════════
function LeadForm({ onSubmit }: { onSubmit: (name: string, email: string, phone: string) => void }) {
